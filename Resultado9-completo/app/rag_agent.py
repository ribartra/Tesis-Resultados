...
class RAGAgent(Agent):
    ...
    def __init__(
        self,
        model_id: str = "Llama-3.2-3B-Instruct_Q4_K_M:latest",
        ...
        system_prompt: str = None,  # System prompt personalizado (None = usar default Qhali)
        ...
        Args:
            ...
            system_prompt: Prompt del sistema personalizado. Si es None, usa el default de Qhali.
                          Si es "", el agente no tendrá system prompt (útil para evaluaciones justas)
        ...
        # Determinar cómo inicializar Agent según system_prompt
        if system_prompt is None:
            # Default: Prompt de Qhali completo
            instructions_text = dedent("""
Eres un asistente de consultas generales de salud, respondes siempre en formato oración con información importante, de forma agradable validando consulta del usuario. Sin embargo, si se te solicita reescribir consultas reescribes adecuadamente en el número de reescrituras que te solicitan generando solo variantes de la consulta como consultas extractivas.
            """)
            # Inicializar con instructions
            super().__init__(
            ...
            # Vacío: NO especificar instructions para evaluación justa
            # Esto permite que Ollama use el system prompt definido en el Modelfile
            super().__init__(
                model=Ollama(id=model_id),
                show_tool_calls=show_tool_calls,
                markdown=markdown
                # NO se pasa instructions → queda en None
            )
        else:
            # Personalizado: usar el prompt proporcionado
            super().__init__(
                model=Ollama(id=model_id),
    ...
    def _sanitize_fts_query(self, query):
        ...
        # Quitar caracteres más conflictivos para el parser FTS
        # CRÍTICO: incluir ':' y '-' que causan Syntax Error en subconsultas avanzadas
        for ch in ["?", "!", "'", '"', "(", ")", ":", "-", "/", "\\", "[", "]", "{", "}"]:
    ...
    def _combine_results(self, results_semantic, results_keyword, alpha):
        ...
        try:
            ...
            if not results_semantic.is_empty() and not results_keyword.is_empty():
                ...
                results_combined = results_semantic.select(semantic_cols)
            elif not results_keyword.is_empty():
                results_combined = results_keyword.select(keyword_cols)
            else:
                return pl.DataFrame()

            # CRÍTICO: Asegurar columnas base ANTES de normalizar (fix para schema inconsistency en ADVANCED mode)
            if "similitud" not in results_combined.columns:
                results_combined = results_combined.with_columns(pl.lit(0.0).alias("similitud"))
            if "score_BM25" not in results_combined.columns:
                results_combined = results_combined.with_columns(pl.lit(0.0).alias("score_BM25"))
            ...
            # SIEMPRE normalizar (garantiza esquema consistente para concat posterior en _merge_across_queries)
            results_combined = results_combined.with_columns([
    ...
    def responder_con_rag_streaming(self, query: str, historial_externo: str = None) -> Iterator[str]:
        ...
        # SOLO si no se proporcionó historial externo (significa que estamos usando el historial interno)
        if historial_externo is None:
            self._actualizar_historial(query, respuesta)
    ...
    def responder_con_rag_streaming_avanzado(self, query: str, historial_externo: str = None) -> Iterator[dict]:
        ...
        if not contexto:    
            ...
            # Si hay contexto, SIEMPRE generar respuesta con el LLM usando el contexto
            # Construcción del prompt con historial y contexto
            prompt = self._construir_prompt(query, contexto, historial_externo)
    ...
    def _retrieve_for_single_query(self, q: str, top_k_sem: int = None, top_k_kw: int = None, alpha: float = None) -> pl.DataFrame:
        """Reutiliza la canalización híbrida para una subconsulta (sin combinar aún)."""
        # Usar valores por defecto si no se proporcionan
        top_k_sem = top_k_sem or self.top_k_semantic
        top_k_kw = top_k_kw or self.top_k_keyword
        alpha = self.alpha if alpha is None else alpha
    ...
    def _merge_across_queries(self, list_of_dfs: List[pl.DataFrame]) -> pl.DataFrame:
        ...
        # Normalizar esquema: quedarnos sólo con las columnas necesarias antes de concat
        base_cols = ["chunk_key", "pdf_name", "chunk_index", "text", "titulo", "fuente", "score_final"]
        dfs_norm = []
        for df in dfs:
            # Asegurar chunk_key si no existe
            ...
            if not {"chunk_key", "score_final"}.issubset(df.columns):
                print(f"[DEBUG] DF sin columnas base en _merge_across_queries: {df.columns}")
                continue
            
            # Seleccionar solo columnas que existen
            available_cols = [c for c in base_cols if c in df.columns]
            dfs_norm.append(df.select(available_cols))
        
        if not dfs_norm:
            return pl.DataFrame()
        
        # Ahora concat con esquemas consistentes
        df = pl.concat(dfs_norm, how="vertical", rechunk=True)
    ...
    def _advanced_search(self, query: str, similarity_threshold: float = None, alpha: float = None):
        ...
        try:
            ...
            dfs = [
                self._retrieve_for_single_query(
                    sub,
                    top_k_sem=self.adv_top_k_per_query,
                    top_k_kw=self.adv_top_k_per_query,
                    alpha=alpha,
                )
                for sub in subs
            ]
            ...
            print(f"⚠️ ZeroDivisionError en _advanced_search: {e}. Devolviendo None.")
            return None
    ...
    def _advanced_search_with_score(self, query: str, similarity_threshold: float = None, alpha: float = None):
        ...
        try:
            ...
            print(f"⚠️ ZeroDivisionError en _advanced_search_with_score: {e}. Devolviendo None.")
            return None
    ...
    def retrieve_only_advanced(self, query: str, top_k_per_query: int = None, alpha: float = None):
        ...
        dfs = [
            self._retrieve_for_single_query(
                s,
                top_k_sem=top_k_per_query,
                top_k_kw=top_k_per_query,
                alpha=alpha,
            )
            for s in subs
        ]
